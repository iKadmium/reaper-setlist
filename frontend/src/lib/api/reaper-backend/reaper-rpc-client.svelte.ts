...
export class ReaperRpcClient {
	...
	public async listProjects(): Promise<{ projects: string[], ok: boolean }> { 
		const commands = [ 
			new SetOperationCommand("listProjects"), 
			new RunScriptCommand(), 
			new GetStateCommand(SectionKeys.ReaperSetlist, "projects"), 
			new GetStateCommand(SectionKeys.ReaperSetlist, "ok"), 
		] as const; 
		const [_setOperation, _runScript, projectsRaw, okRaw] = await this.apiClient.executeCommands(commands);  
		...
		if (okRaw === undefined) { 
			throw new Error("Failed to retrieve ok. Please check the script configuration."); 
		} 
		const ok = okRaw === 'true';  
		return { projects, ok };  
	}  
