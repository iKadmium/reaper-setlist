...
FROM ghcr.io/rust-cross/rust-musl-cross:${RUSTARCH}-musl AS backend-builder
ARG RUSTARCH
...
RUN cargo build --target ${RUSTARCH}-unknown-linux-musl --release
COPY ./backend/ ./
RUN cargo build --target ${RUSTARCH}-unknown-linux-musl --release

# Move the compiled binary to a generic, architecture-agnostic path
RUN cp ./target/${RUSTARCH}-unknown-linux-musl/release/reaper_setlist_backend /app/reaper_setlist_backend_final
